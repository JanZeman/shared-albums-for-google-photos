					...
					<table class="jzsa-settings-table jzsa-settings-table--params">
						...
						<tbody>
							...
							<tr>
								...
								<td><?php echo wp_kses_post( __( '<p>Controls how visitors enter fullscreen. Use <code>button-only</code> (default) to require the fullscreen button, <code>click</code> for a single-click shortcut, <code>double-click</code> for a double-click shortcut, or <code>disabled</code> to turn fullscreen off.</p><p><code>double-click</code> is usually the best gesture option because single-click navigation still works in fullscreen. With <code>click</code>, mouse users cannot click left or right to browse while fullscreen is active.</p><p><code>lightbox-toggle</code> and <code>fullscreen-toggle</code> can both be active. Visitors then see both buttons and choose which expanded view to open. Use <code>fullscreen-toggle="disabled"</code> for lightbox-only behaviour.</p>', 'janzeman-shared-albums-for-google-photos' ) ); ?></td>
								<td>button-only</td>
							</tr>
							<tr>
