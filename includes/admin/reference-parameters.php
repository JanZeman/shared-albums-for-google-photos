					...
					<p><?php echo wp_kses_post( __( 'The lightbox is an alternative to native fullscreen. Instead of the browser taking over the whole screen, clicking a photo opens it in a dimmed overlay <strong>on top of the page</strong>, in a size-capped box. Lightbox is now the default expanded view unless you explicitly set fullscreen. You can still show both side by side with <code>expanded-toggle="lightbox-button, fullscreen-button"</code>; visitors then choose which experience they prefer. <code>interaction-lock="true"</code> disables both.', 'janzeman-shared-albums-for-google-photos' ) ); ?></p>
					<p><?php echo wp_kses_post( __( 'Some lightbox display settings are paired with their <code>fullscreen-*</code> counterparts. Set only one of the pair and the other inherits it automatically; set both when the two expanded views should behave differently. The rows below name the paired parameter where this applies.', 'janzeman-shared-albums-for-google-photos' ) ); ?></p>
					<p><?php echo wp_kses_post( __( 'For a concrete example, see Guide sample 34: Lightbox + Fullscreen - Both Buttons Side by Side. It sets <code>fullscreen-controls-color="#E63946"</code> but no <code>lightbox-controls-color</code>, so both expanded views use red controls. Add <code>lightbox-controls-color="green"</code> and the lightbox controls become green while fullscreen stays red.', 'janzeman-shared-albums-for-google-photos' ) ); ?></p>
					<table class="jzsa-settings-table jzsa-settings-table--params">
						<thead>
							<tr>
								<th>Parameter</th>
						...
						<tbody>
							<tr>
								...
								<td><?php echo wp_kses_post( __( 'How the lightbox is opened: <code>button-only</code> (default), <code>click</code> (a single click on the photo opens it), <code>double-click</code> (a double-click opens and closes it), or <code>disabled</code> to turn it off. Any value other than <code>disabled</code> enables the lightbox. The site now defaults to Lightbox when no expanded view is set explicitly. The fullscreen button is hidden unless you enable the fullscreen side yourself. Note: like the fullscreen side, <code>click</code> uses clicks to close the lightbox, so click-to-navigate is unavailable in that mode - <code>double-click</code> is the friendlier choice when visitors also click to browse.', 'janzeman-shared-albums-for-google-photos' ) ); ?></td>
								<td>disabled</td>
							</tr>
							<tr>
