...
function jzsa_add_privacy_policy_content() {
	...
	$content .= '<ul>';
	$content .= '<li><strong>' . esc_html__( 'Email sign-in.', 'janzeman-shared-albums-for-google-photos' ) . '</strong> ' . esc_html__( 'When you sign in to the community, the email address you enter is sent to jzsa.janzeman.com so the community server can send a one-time confirmation link and identify your account. It is used only for sign-in and account-related messages, not newsletters or marketing.', 'janzeman-shared-albums-for-google-photos' ) . '</li>';
	...
	$content .= '<li><strong>' . esc_html__( 'Published entry data.', 'janzeman-shared-albums-for-google-photos' ) . '</strong> ' . esc_html__( 'If you publish an entry, the following is stored on jzsa.janzeman.com: title, shortcode settings, the extracted Google Photos album link, optional description, optional tags, optional sample page URL, optional photographer or creator name, optional short bio, plugin version, whether you submitted the page for future public site showcase consideration, and whether you allow the shortcode settings to be shown there. You control all of this data.', 'janzeman-shared-albums-for-google-photos' ) . '</li>';
